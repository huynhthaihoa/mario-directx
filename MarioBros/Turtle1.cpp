...
Turtle1::Turtle1(void):BaseObject()
{
}

void Turtle1::Init()
{
	this->position = D3DXVECTOR3(300,210,0);
...
void Turtle1::Load(Contents* content){
	this->img = content->LoadTexture("./Monster.png");
}
void Turtle1::Update(KeyBoard* keyboard,DWORD dt)
{
	if(checkLR == false)
	{
		if(position.x  >= 300)
...
void Turtle1::Draw(Graphic*g)
{
	g->DrawTexture(img,&bound,this->position,D3DCOLOR_XRGB(255,255,255));
}

void Turtle1::OnCollisionWith(BaseObject* obj)
{
	//xu li va cham o day
