...
void CGameTimeDx9::UpdateGameTime()
{
	...
	if( (UINT64)((CurTicks - LastTicks) / m_FreQuency) 
		< CTimeSpanDx9::TicksPerMillisecond)
	{
		return;
	}
	else
		{
			this->setElapsedGameTime( 
				CTimeSpanDx9( 
					(UINT64)((CurTicks - LastTicks) / m_FreQuency ) 
					));
			LastTicks = CurTicks;
		}
}
...
{
	if (m_ElapsedGameTime.getMilliseconds() > 1000.0 / 60)
	{
		return CTimeSpanDx9( 1000.0 / 60 );
	}
